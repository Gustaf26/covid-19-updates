...
listoutput = (dat) => {let dataarr = [...this.state.data];
   ...
   for (j=0; j<dat.data.covid19Stats.length; j++) {
    ...
    usarr.push({country:dat.data.covid19Stats[j].country, city: dat.data.covid19Stats[j].city, province: dat.data.covid19Stats[j].province, confirmed: dat.data.covid19Stats[j].confirmed, recovered: dat.data.covid19Stats[j].recovered, deaths: dat.data.covid19Stats[j].deaths, timestamp: dat.data.covid19Stats[j].lastUpdate})
   }

  
   //let doubleselectarr = selectarr.filter(cit=>cit.city =="")
   //console.log(globalarr)
   let sortedarr= usarr.filter(pers=>pers.deaths > 1000)
